...
const createDazzleTestConfig = require('dazzle/config/createDazzleTestConfig');
const plugin = require('../index');
const { babelLoaderFinder, tsLoaderFinder } = require('../helpers');

describe('dazzle-typescript-plugin', () => {
  describe('with useBabel=false', () => {
    let config;
    beforeAll(async done => {
      config = await createDazzleTestConfig('web', 'dev', {
  ...
  describe('when creating a server config', () => {
    let config;
    beforeAll(async done => {
      config = await createDazzleTestConfig('node', 'dev', {
