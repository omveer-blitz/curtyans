...
const dazzleBabelPreset = require('babel-preset-dazzle');
...
const dazzleBabelPresetModern = function(presetOptions) {
  return function(context) {
    return dazzleBabelPreset(context, getModernOptions(presetOptions));
  };
};

module.exports = babelLoader.custom(function(babel) {
  const presetItem = babel.createConfigItem(dazzleBabelPreset, {
    type: 'preset',
  });
  const applyCommonJs = babel.createConfigItem(
    require('babel-preset-dazzle/babel-plugins/commonjs'),
    { type: 'plugin' }
  );
  ...
  return {
      ...
      const loader = Object.assign(
        opts.cache
          ? {
              ...
              cacheDirectory: join(opts.cwd, 'cache', 'dazzle-babel-loader'),
              cacheIdentifier:
                cacheKey +
    ...
    config: function(cfg, cfgOpts) {
      ...
      // RR-TODO: Removing as it is unstable and does not support require()/commonjs 
      ...
      //       [
      //         require('babel-preset-dazzle/babel-plugins/no-anonymous-default-export'),
      //         {},
      //       ],
      //       { type: 'plugin' }
      //     );
      ...
      if (isModern) {
        const razzlePreset = presetOptions.presets.find(
          preset => preset && preset.value === dazzleBabelPreset
        ) || { options: {} };
        ...
        const presetItemModern = babel.createConfigItem(
          dazzleBabelPresetModern(razzlePreset.options),
          {
            type: 'preset',
          }
        );
