...
const StartServerPlugin = require('dazzle-start-server-webpack-plugin');
...
const resolveRequest = require('dazzle-dev-utils/resolveRequest');
const logger = require('dazzle-dev-utils/logger');
const dazzlePaths = require('./paths');
const getCacheIdentifier = require('react-dev-utils/getCacheIdentifier');
const webpackMajor = require('dazzle-dev-utils/webpackMajor');
...
if (devServerMajorVersion > 3) {
  webpackDevClientEntry = require.resolve('dazzle-dev-utils/webpackHotDevClientV4');
} else {
  webpackDevClientEntry = require.resolve('dazzle-dev-utils/webpackHotDevClient');
}
...
module.exports = (
  ...
  return new Promise(async resolve => {
    ...
    const IS_SERVERLESS = /serverless/.test(dazzleOptions.buildType);
    ...
    const shouldUseReactRefresh =
      IS_WEB && IS_DEV && dazzleOptions.enableReactRefresh ? true : false;

    const shouldDisableWebpackbar =
      dazzleOptions.disableWebpackbar === true || dazzleOptions.disableWebpackbar === target;
    ...
    const dotenv = getClientEnv(
      target, IS_DEV,
      { clearConsole, host, port, shouldUseReactRefresh, forceRuntimeEnvVars: dazzleOptions.forceRuntimeEnvVars, webpackObject },
      paths
    );
    ...
    webpackOptions.fileLoaderOutputName = `${dazzleOptions.mediaPrefix}/[name].[contenthash:8].[ext]`;

    webpackOptions.urlLoaderOutputName = `${dazzleOptions.mediaPrefix}/[name].[contenthash:8].[ext]`;

    webpackOptions.cssTest = [/\.css(\.map)?$/];

    webpackOptions.cssOutputFilename = `${dazzleOptions.cssPrefix}/[name].[contenthash:8].css`;

    webpackOptions.cssOutputChunkFilename = `${dazzleOptions.cssPrefix}/[name].[contenthash:8].chunk.css`;
    ...
    if (IS_WEB) {
      ...
      if (IS_DEV) {
        ...
        webpackOptions.jsOutputFilename = `${dazzleOptions.jsPrefix}/[name].js`;
        webpackOptions.jsOutputChunkFilename = `${dazzleOptions.jsPrefix}/[name].chunk.js`;

        webpackOptions.splitChunksConfig = splitChunksConfigs.dev;
      } else {

        webpackOptions.jsOutputFilename = `${dazzleOptions.jsPrefix}/[name].[contenthash:8].js`;
        webpackOptions.jsOutputChunkFilename = `${dazzleOptions.jsPrefix}/[name].[contenthash:8].chunk.js`;

        webpackOptions.splitChunksConfig = splitChunksConfigs.prod;
        webpackOptions.terserPluginOptions = {
          terserOptions: {
            // RR-TODO: Recheck these settings later to optimise
            parse: {
            ...
            // RR-TODO: With latest terser-webpack-plugin, this had to be included inside terserOptions
            // @todo add flag for sourcemaps
            sourceMap: dazzleOptions.enableSourceMaps,
          },
          // @todo add flag for sourcemaps
          // sourceMap: dazzleOptions.enableSourceMaps,
        };
    ...
    webpackOptions.browserslist = dazzleOptions.browserslist;

    webpackOptions.babelRule = {
        test: /\.(js|jsx|mjs|ts|tsx)$/,
        include: [paths.appSrc].concat(additionalIncludes),
        use: [{
          loader: require.resolve('./babel-loader/dazzle-babel-loader'),
          options: {
            verbose: dazzleOptions.verbose,
            sourceMaps: dazzleOptions.enableSourceMaps,
            isServer: IS_NODE,
            cwd: paths.appPath,
            cache: dazzleOptions.enableBabelCache,
            configFile: dazzleOptions.enableTargetBabelrc ? path.resolve(paths.appPath, `.babelrc.${target}`) : undefined,
            hasModern: false,
            development: IS_DEV,
            shouldUseReactRefresh: shouldUseReactRefresh,
          },
          ident: 'dazzle-babel-loader'
        }
      ]
    };
    ...
    webpackOptions.postCssOptions = {
      ...
      plugins: [
        [require('autoprefixer'), {
          overrideBrowserslist: dazzleOptions.browserslist || [
            '>1%',
            'last 4 versions',
            'Firefox ESR',
            'not ie < 9',
    ...
    // Check if dazzle.config.js has a modifyWebpackOptions function.
    ...
    let config = {
      ...
      devtool: IS_DEV || IS_DEV_ENV ? 'cheap-module-source-map' : dazzleOptions.enableSourceMaps ? 'source-map' : false,
      // We need to tell webpack how to resolve both Dazzle's node_modules and
      ...
      resolve: {
        ...
        plugins: [
          // RR-TODO: Since v1.7.0 of PnpWebpackPlugin supports webpackMajor 5,
          // removing this check now
          ...
          // Note: this yields the exact same CSS config as create-react-app.
          {
            ...
            [
              webpackOptions.nullNodeCss ? {
                ...
                ident: 'dazzle-css-loader'
              },
            ]
            : IS_DEV
            ? [
              dazzleOptions.staticCssInDev
                ?  {
                  loader: MiniCssExtractPlugin.loader,
                  ident: 'dazzle-mini-css-extract-loader'
                } : {
                  loader: require.resolve('style-loader'),
                  ident: 'dazzle-style-loader'
                },
              {
                loader: require.resolve('css-loader'),
                options: {
    ...
    if (IS_NODE) {
      ...
      if (IS_PROD) {
        ...
        if (webpackMajor === 5) {
          config.optimization.emitOnErrors = dazzleOptions.emitOnErrors;
        } else {
          config.optimization.noEmitOnErrors = !dazzleOptions.emitOnErrors;
        }
      ...
      if (IS_DEV) {
        ...
        config.entry.server.unshift(
          require.resolve('dazzle-dev-utils/prettyNodeErrors')
        );
      ...
      if (IS_DEV) {
        ...
        if (devServerMajorVersion > 3) {
          ...
          config.devServer = Object.assign(config.devServer, {
            ...
            // RR-TODO: New webpack v5 replaced onBeforeSetupMiddleware/onAfterSetupMiddleware with
            // setupMiddlewares
        ...
        config.plugins = [
          ...
          // RR-TODO: Removing this as the package is unstable
          // and does not support require()/common-js
          ...
          //   : null,
          new webpack.DefinePlugin(webpackOptions.definePluginOptions),
          dazzleOptions.staticCssInDev ? miniCssExtractPlugin : null,
        ].filter(x => x);
          ...
          config.optimization = {
            ...
            minimizer: [
              ...
              new CssMinimizerPlugin({
                // RR-TODO: With css-minimizer-webpack-plugin v7.0.0, this is no longer supported
                // sourceMap: dazzleOptions.enableSourceMaps,
                minimizerOptions: {
                  sourceMap: dazzleOptions.enableSourceMaps
                },
                minify: async (data, inputMap, minimizerOptions) => {
                  ...
                  return {
                    code: minifiedCss.styles, // RR-TODO: added this for new version of CssMinimizerPlugin
    ...
    // Check if dazzle.config.js has a modifyWebpackConfig function.
    // If it does, call it on the configs we created.
