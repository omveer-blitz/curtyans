...
// Grab NODE_ENV and DAZZLE_* environment variables and prepare them to be
// injected into the application via DefinePlugin in Webpack configuration.
const DAZZLE = /^DAZZLE_/i;

function getClientEnvironment(target, is_dev, options, paths) {
  const raw = Object.keys(process.env)
    .filter(key => DAZZLE.test(key))
    .reduce(
      (env, key) => {
      ...
      },
      {
        ...
        DAZZLE_ASSETS_MANIFEST: paths.appAssetsManifest,
        ...
        // It's useful if you're running dazzle in a non-localhost container. Ends in a /
