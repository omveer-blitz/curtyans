...
const loadDazzleConfig = require('../config/loadDazzleConfig');
const printErrors = require('dazzle-dev-utils/printErrors');
const printWarnings = require('dazzle-dev-utils/printWarnings');
const clearConsole = require('react-dev-utils/clearConsole');
const logger = require('dazzle-dev-utils/logger');
const FileSizeReporter = require('dazzle-dev-utils/FileSizeReporter');
const formatWebpackMessages = require('dazzle-dev-utils/formatWebpackMessages');
...
loadDazzleConfig(webpack).then(
  async ({ dazzle, dazzleOptions, webpackObject, plugins, paths }) => {
    const verbose = dazzleOptions.verbose;
    if (!verbose) {
      process.removeAllListeners('warning');
    }
    if (!process.env.CI && process.env.NODE_ENV === "production" && (process.env.DAZZLE_NONINTERACTIVE !== "true" && !cliArgs['noninteractive'])) {
    ...
    const clientOnly = /spa|single\-page\-application/.test(
      dazzleOptions.buildType
    );
    const serverOnly = /serveronly|server\-only/.test(dazzleOptions.buildType);

    process.env.BUILD_TYPE = dazzleOptions.buildType;
              ...
              return new Promise((resolveBuild, rejectBuild) => {
                const build = async previousFileSizes => {
                  return new Promise(async (resolve, reject) => {
                    // Create our production webpack configurations and pass in dazzle options.
