...
// Wrapper over asyncPool to mitigate code change 
// for bumping tiny-async-pool to latest version
...
loadDazzleConfig(webpack).then(
  async ({ dazzle, dazzleOptions, webpackObject, plugins, paths }) => {
    ...
    async function static_export(previousFileSizes) {
      const options = dazzleOptions.staticExport || {};
      ...
      const rendersInfo = await asyncPoolAll(
      ...
      const insertScriptCode =
        `window.${options.windowRoutesVariable || 'DAZZLE_STATIC_ROUTES'}` +
        ` =  ${JSON.stringify(
          routes.map(route => route.replace(/^\/|\/$/g, ''))
        )};\n` +
        `window.${options.windowRoutesDataVariable ||
          'DAZZLE_STATIC_DATA_ROUTES'}` +
        ` =  ${JSON.stringify(
