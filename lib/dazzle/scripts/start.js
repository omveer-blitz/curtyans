      ...
      .then(
        async ({ dazzle, dazzleOptions, webpackObject, plugins, paths }) => {
          ...
          setPorts(clientOnly).then(async () => {
            ...
            let clientCompiler;
            let clientConfig; // Create dev configs using our config factory, passing in dazzle file as
            // options.
            if (!serverOnly) {
            ...
            if (clientCompiler) {
              clientCompiler.hooks.done.tap('dazzle', () => {
